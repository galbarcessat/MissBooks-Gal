...
function _createBook(title = 'test', description = 'bla bla bla', thumbnail, listPrice) {
...
function deleteReview(book, reviewFullName) {
  let bookReviews = book.reviews
  let reviewIndex = bookReviews.findIndex((review) => review.fullname === reviewFullName)
  if (reviewIndex !== -1) {
...
function addGoogleBook(googleBook) {
  ...
  let { title, language, publishedDate, pageCount, categories, description, imageLinks, authors } = volumeInfo
  return Promise.resolve({
    ...
    pageCount,
    categories,
    thumbnail: imageLinks ? imageLinks.thumbnail : '../assets/BooksImages/1.jpg',
    language,
    listPrice: {
      amount: getRandomIntInclusive(10, 30),
      currencyCode: 'EUR',
      isOnSale: false,
    },
  })
